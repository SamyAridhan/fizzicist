...
const ss = {
  app: {
    minHeight: "100vh", background: C.bg, maxWidth: 480, margin: "0 auto",
...
// ─────────────────────────────────────────────
// ADD PRODUCT MODAL
// FIX: removed rawFile (unused), changed `image` → `preview`, wired setSaving correctly
// ─────────────────────────────────────────────
function AddProductModal({ onAdd, onClose }) {
  ...
  const [err, setErr] = useState("");
  const [saving, setSaving] = useState(false);  // FIX: kept, now actually used below

  const handleImg = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    // FIX: removed setRawFile — we only need the compressed preview
  ...
  const submit = async () => {
    ...
    setSaving(true); // FIX: now used — disables button during upload
    try {
      const id = Date.now().toString();
      let image_url = null;

      if (preview) {
        // FIX: was `image` (undefined) — now correctly uses `preview`
      ...
      const product = {
        ...
        image_url,
        image: image_url, // alias used by ProductCard's <img src>
      };
  ...
  return (
    <Modal title="Upload Receipt" onClose={onClose}>
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <div style={{ fontSize: 13, color: C.muted }}>Take a photo or upload a screenshot of the receipt</div>
      </div>
      ...
      {preview ? (
        ...
        <div style={{ border: `2px dashed ${C.greenMid}`, borderRadius: 12, padding: "28px 16px", textAlign: "center", marginBottom: 14 }}>
          <input type="file" accept="image/*" capture="environment" onChange={handleImg}
            id="receipt-img" style={{ display: "none" }} />
      ...
      {!preview && (
        <div style={{ fontSize: 12, color: C.hint, textAlign: "center", marginTop: 8 }}>
          Receipt is optional — you can skip if needed
        </div>
      )}
    </Modal>
  );
...
// ─────────────────────────────────────────────
// DASHBOARD
// FIX: handleConfirm now calls onTransaction (the prop) not saveTransaction
//      handleDeleteProduct is now used for the onDelete prop (removed duplicate inline)
//      onTransaction prop is now consumed correctly
// ─────────────────────────────────────────────
function Dashboard({ products, setProducts, cart, setCart, user, onTransaction, showToast }) {
  ...
  // FIX: was calling `saveTransaction` (undefined in this scope).
  // Correct: call `onTransaction` which is the prop passed from root App.
  // FIX: Date.now() moved inside useCallback-wrapped async handler — no render-time impurity.
  const handleConfirm = useCallback(async (receiptDataUrl) => {
    ...
    await onTransaction(txn); // FIX: was `saveTransaction(txn)` — not in scope
    setCart([]);
    setShowReceipt(false);
    setShowCart(false);
    showToast("Transaction saved!");
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cart, cartTotal, user, onTransaction, showToast]);

  // FIX: defined once and passed as `onDelete` prop — was defined inline AND here, causing unused-var warning
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
          {products.map((p) => (
            <ProductCard
              ...
              onDelete={() => handleDeleteProduct(p.id)} // FIX: use the single handler defined above
            />
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        {filtered.length > 0 && (
          <button onClick={() => exportCSV(filtered, filterLabel)} style={{
...
// ─────────────────────────────────────────────
// ROOT APP
// FIX: setDbError is now used inside the useEffect catch block
// FIX: removed saveProducts (not needed — product changes go straight to Supabase + local state)
// ─────────────────────────────────────────────
export default function App() {
  ...
  const [dbError, setDbError] = useState(false); // FIX: now actually set in catch below
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setDbError(true); // FIX: was never called before — now wires up the error screen
      } finally {
        setLoading(false);
      }
    })();
  ...
  // FIX: saveProducts removed entirely — not needed.
  // Products are saved to Supabase inside AddProductModal.submit() and
  // deleted inside Dashboard.handleDeleteProduct(). Local state is updated
  // via setProducts passed as a prop. No intermediate saveProducts needed.
    ...
    return (
      <div style={{
        ...
        <div style={{ fontSize: 13, color: C.muted, maxWidth: 300 }}>
          Check your Supabase environment variables and network connection, then refresh.
        </div>
